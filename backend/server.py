...
async def websocket_log_callback(log_entry: SystemLog):
    ...
    message = {
        "type": "log",
        "data": jsonable_encoder(log_entry.dict())
    }
    await manager.broadcast(json.dumps(message, cls=DateTimeEncoder))
        ...
        while config.get('status') == SystemStatus.RUNNING:
            try:
                ...
                if slots:
                    ...
                    message = {
                        "type": "slots_found",
                        "data": {"slots": [jsonable_encoder(slot.dict()) for slot in slots]}
                    }
                    await manager.broadcast(json.dumps(message, cls=DateTimeEncoder))
